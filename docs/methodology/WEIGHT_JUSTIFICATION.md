...
## A Circularity in the Weighting

*Added v4.0.1 (August 2026). The weights above carried unchanged into v4.0, so this applies to the published index.*

One of our three weighting factors is **deflationary force** — the size of a sector's cost decline. That factor sits on both sides of the calculation. Sectors that fell fastest receive larger weights, and larger weights steepen the composite. In principle, an index weighted in part by what it measures can report a deeper decline than one weighted on external grounds alone. A reader should not have to find that on their own.

**The case for keeping it.** The index measures the input layer, and a sector's weight is meant to capture how much of the economy runs through it. Deflationary force is a usable proxy: inputs that collapse in cost get built into everything downstream, which is why compute and bandwidth are load-bearing in a way that industrial gases are not. Weighting on GDP share alone would understate exactly the sectors whose cheapness is the subject.

**The case against.** That reasoning is a description of *enabling effect*, which is already a separate factor. Deflationary force may be double-counting it, and the side effect is a mechanically steeper headline.

We have not resolved this. What we can do is show its size. The no-deflationary-force variant removes the factor entirely and redistributes its share to the two external factors — an equal blend of the GDP-contribution-only and enabling-effect-only vectors documented below (computing 35%, communications 17.5%, energy 30%, transportation 17.5%) — keeping everything else identical to the published v4.0 build: same sector series, same geometric aggregation, same late-start convention.

| Variant | Weights (C / Co / E / T) | Composite (1990–2025) | Annual |
|---|---|---|---|
| v4.0 published weights | 29.4 / 23.5 / 29.4 / 17.6 | −99.97% | −20.4% |
| Deflationary force removed, share redistributed to GDP contribution and enabling effect | 35 / 17.5 / 30 / 17.5 | −99.98% | −21.1% |

The result cuts against the obvious suspicion: removing the deflationary-force factor makes the composite slightly **deeper**, not shallower. The share it gives up flows mostly to computing — the sector the two external factors rate highest — which is also the fastest-falling series. The circularity is real as a matter of construction, but it is not what drives the headline; on these weights it modestly moderates it. The full computation is published in [`data/v4/sensitivity_no_df.json`](../../data/v4/sensitivity_no_df.json), generated by [`scripts/sensitivity_no_df.py`](../../scripts/sensitivity_no_df.py).

If you think the weighting is doing too much work, the second row is the number to argue with.
...
**Changes**: Added "A Circularity in the Weighting" — disclosure and no-deflationary-force sensitivity (v4.0.1). Previous (3.0, January 2026): added comprehensive BEA/GDP data section with citations; strengthened academic foundation
**Next Review**: none planned — project complete through 2025
